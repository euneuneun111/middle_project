...
public class ReplyDAOImpl implements ReplyDAO{
	...
	@Override
	public List<ReplyVO> selectReplyList(int fno,PageMaker pageMaker) throws SQLException {		
		int offset = pageMaker.getStartRow()-1;
		int limit = pageMaker.getPerPageNum();
		
		RowBounds bounds = new RowBounds(offset,limit);
		
		return session.selectList("Reply-Mapper.selectReplyList",fno,bounds);
	}
	@Override
	public int countReply(int fno) throws SQLException {		
		return session.selectOne("Reply-Mapper.countReply",fno);
	}
	@Override
